...
// =======================================================================================
// Unit Testing Helpers (only with OpenTelemetry + unit test builds)
// =======================================================================================

#if defined(UNIT_TESTING) && defined(ENABLE_OTEL)

/**
 * @brief Reset all primer state for unit testing.
 *
 * Clears all primer maps and done sets. This function is used by unit tests to reset
 * the primer state between test cases.
 */
void test_resetPrimerState();

/**
 * @brief Get the current state of a collective primer.
 */
PrimerState test_getCollectivePrimerState(CommunicatorState* commState, const std::string& key);

/**
 * @brief Set the state of a collective primer (for testing).
 */
void test_setCollectivePrimerState(CommunicatorState* commState, const std::string& key, PrimerState state);

/**
 * @brief Check if a collective primer is marked as done.
 */
bool test_isCollectivePrimerDone(CommunicatorState* commState, const std::string& key);

/**
 * @brief Mark a collective primer as done (for testing).
 */
void test_markCollectivePrimerDone(CommunicatorState* commState, const std::string& key);

/**
 * @brief Get the total number of collective primers.
 */
size_t test_getCollectivePrimerCount();

/**
 * @brief Get the total number of done collective primers.
 */
size_t test_getCollectivePrimerDoneCount();

/**
 * @brief Get the current state of a P2P primer.
 */
PrimerState test_getP2PPrimerState(CommunicatorState* commState, const std::string& key);

/**
 * @brief Get the current state of a rank transfer primer.
 */
PrimerState test_getRankPrimerState(CommunicatorState* commState, const std::string& key);

/**
 * @brief Get the current state of a channel transfer primer.
 */
PrimerState test_getTransferPrimerState(CommunicatorState* commState, const std::string& key);

/**
 * @brief Get the windows waited count for a collective primer.
 */
uint32_t test_getCollectivePrimerWindowsWaited(CommunicatorState* commState, const std::string& key);

#endif  // UNIT_TESTING && ENABLE_OTEL
